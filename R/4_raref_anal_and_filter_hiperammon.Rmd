...
#1st step filtering(=0 all arrive to saturation)
ps_filtered1<-subset_samples(ps_filtered1, SampleID != "PC249_S31")
...
Prevdf<- apply(X = otu_table(ps_filtered1),
               MARGIN = 1,
               FUN = function(x){sum(x > 0)})

##Add taxonomy and total read counts to this data.frame
Prevdf<- data.frame(Prevalence = Prevdf,
                    TotalAbundance = taxa_sums(ps_filtered1),
                    tax_table(ps_filtered1))

plyr::ddply(Prevdf, "Phylum", function(df1){
  data.frame(mean_prevalence=mean(df1$Prevalence),total_abundance=sum(df1$TotalAbundance,na.rm = T),stringsAsFactors = F)
})

ggplot(Prevdf, aes(TotalAbundance, Prevalence / nsamples(ps_filtered1),color=Phylum)) +
...
wh0 <- genefilter_sample(ps_filtered1, filterfun_sample(function(x) x > 5), A=0.1*nsamples(ps_filtered1))
ps_filtered2<- prune_taxa(wh0, ps_filtered1)
