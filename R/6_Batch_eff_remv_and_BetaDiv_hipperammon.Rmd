...
metadata_hiper_h<-as.data.frame(sample_data(ps))
...
library(vegan)
library(usedist)
euclidean_matrix<- ps@otu_table %>%
    t() %>%
    as.data.frame() %>%
    # euclidean distances
    vegan::vegdist(method = 'euclidean',
                   diag = TRUE, upper = TRUE)
metadata_hiper<-as.data.frame(sample_data(ps))
