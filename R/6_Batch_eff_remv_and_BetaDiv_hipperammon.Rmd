...
rownames(SAMPLE) <-metadata_hiper$SampleID
 ...
 ps_limma
saveRDS(ps_limma,"~/Documents/Lola/Datos_CIPF_2023/Rats_HE_HiperAmmon_model/outputs/ps_limma.Rds")
```
...
metadata_hiper_h$Group[metadata_hiper_h$Group == "Hiperammonemic"] <- "Hyperammonemic"
...
plot(permdisp, main="PCoA clr", hull=FALSE, ellipse=TRUE, segments = TRUE, seg.col = c("#F8766D", "#00B0F6"), col = c("#F8766D", "#00B0F6"), label = TRUE, label.cex = 1.2, pch = c(20, 20, 20), seg.lty = c(1), seg.lwd = 1, cex.lab = 1.2, cex.main =1.3, xlab = paste0('PCoA1 (14.77%)'), ylab = paste0('PCoA2 (8.62 %)'))

permdisp$eig[1:3]/sum(permdisp$eig[permdisp$eig > 0]) * 100
#    PCoA1     PCoA2     PCoA3 
#14.767712  8.619812  7.391269 
```
...
meta <- meta(ps_limma)
permanova <- adonis(t(otu) ~ Group + Batch,
                    data = meta, permutations=999, method = "euclidean", by = "margin")

# P-value
permanova$aov.tab

#PERMANOVA having the Batch factor included 
otu <- data.frame(otu_table(ps_clr))
meta <- data.frame(sample_data(ps_clr))
permanova <- adonis(t(otu) ~ Group, strata = meta$Batch,
                    data = meta, permutations=999, method = "euclidean", by = "margin")
