...
#Data
```{r MetadeconfoundR heatmaps_comp}
library(devtools)
install_github("TillBirkner/metadeconfoundR")

ps_comp <- readRDS("/fast/AG_Forslund/Lola/Datos_CIPF_2023/Rats_HE_HiperAmmon_model/outputs/ps_comp.Rds")
library(readxl)
...
library(dplyr)
ps_comp.rel<-ps_comp
latest_annotations <- get_latest_annotation(ps_comp.rel)
latest_annotations<-as.data.frame(latest_annotations)
short_names<-as.data.frame(otu_table(ps_comp.rel))
rownames(short_names)<-latest_annotations$TaxaID
...
saveRDS(Output1, "/fast/AG_Forslund/Lola/Datos_CIPF_2023/Rats_HE_HiperAmmon_model/outputs/Output1_metadec.Rds")
saveRDS(Output2_batch, "/fast/AG_Forslund/Lola/Datos_CIPF_2023/Rats_HE_HiperAmmon_model/outputs/Output2_batch_all.Rds")
...
effect_table_sig_long%>%
    ggplot(aes (x = Group, y = reorder(rowname, effectSize)))+ 
    geom_point (aes (fill = effectSize, shape = as.factor (sign (effectSize)), size = abs (effectSize), color=(status))) +
    scale_shape_manual (values = c (25, 24, 24)) + 
    scale_fill_gradient2 (low = "blue", high = "red", mid = "white", midpoint = 0) +
    scale_color_manual(values=c("gray22","gray1"), labels = c("Confounded", "Deconfounded"))+
    guides(color = guide_legend(override.aes = list(shape = c(1)))) +
    geom_text (aes (label = stars.pval (corr_p), colour = status), size = 8, key_glyph = "point")+
    theme_classic()  +
    labs(
...
df <- data.frame(Abundance = t(abundances(ps_comp)),
                 Group = meta(ps_comp)$Group)
...
Output2_batch <- MetaDeconfound(featureMat = as.data.frame(Meta_Species),
                                metaMat = as.data.frame(metadata_hiper_R), nnodes = 2, randomVar = list("+ (1|Batch)",
                                                                                                         c("Batch")))
